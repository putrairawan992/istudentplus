...
import { loginAction } from "@/app/(internal)/admin/login/actions";
...
export default function AdminLogin({
  ...
  return (
    <form action={formAction} className="mt-8 flex max-w-sm flex-col gap-2.5 rounded-2xl border border-line bg-card p-5">
      <p className="text-sm font-bold">{copy.heading}</p>
      {/* Back to the forum in the language they were reading it in. */}
      <input type="hidden" name="redirectTo" value={localePath(lang, "/threads")} />
      <input name="username" type="text" autoComplete="username" required placeholder={copy.username} className={inputClass} />
      <input
        name="password"
        type="password"
